...
module.exports = {
    ...
    var eventsPromise = new Promise(function (resolve, reject) {
      request.get(
        ...
        function (err, response, body) {
          ...
          // Add an empty array if the property doesn't exist.
                ...
                try {
                  ...
                  events = events.concat(
                    result.Event.map(function (event) {
                      ...
                      event.lastModifiedDate = arrayTime(
                      ...
                      event.alarms = event.alarms.map(function (alarmGuidId) {
                        return result
                          .Alarm[result.Alarm.indexOfKey(alarmGuidId, "guid")];
                      });
                      if ("Recurrence" in result) {
                        event.recurrence =
                          result.Recurrence[
                            result.Recurrence.indexOfKey(
                              event.recurrence,
                              "guid"
                            )
                          ];
                        if (event.recurrence && "until" in event.recurrence)
                          event.recurrence.until = arrayTime(
                            event.recurrence.until
                          );
                        if (
                          event.recurrence &&
                          "recurrenceMasterStartDate" in event.recurrence
                        )
                          event.recurrence.recurrenceMasterStartDate = arrayTime(
                            event.recurrence.recurrenceMasterStartDate
                          );
                      }
                      return event;
                    })
  ...
  __event(event, all, callback = function () {}, methodOverride) {
    ...
    var eventPromise = new Promise(function (resolve, reject) {
      ...
      if (recurrence && "until" in recurrence)
        recurrence.until = timeArray(recurrence.until);
      ...
      var url =
        ...
        "/" +
        event.guid +
        (event.recurrence ? (all ? "/future" : "/this") : "") +
        "?" +
        paramStr(
          (function () {
            var args = {
...
function getCTag(etag) {
  return (
    "FT=-@RU=" +
    etag.substring(etag.search(/[^-]{8}-[^-]{4}-[^-]{4}-[^-]{4}-[^-]{12}/)) +
    "@S=" +
    etag.substring(etag.search(/[0-9]{1,}/), etag.search("@"))
  );
}
